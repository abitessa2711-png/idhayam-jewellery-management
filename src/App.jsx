...
export default function App() {
  ...
  const deleteSale = async (id) => {
    try {
      // 1. Delete sale entry permanently from sales table
      ...
      if (!delResult || delResult.length === 0) {
        // Fallback if select() is blocked by RLS return: attempt basic delete
        await supabase.from('sales').delete().eq('id', id)
      }

      // 2. Refresh sales state
      await loadData()
      alert("விற்பனைப் பதிவு வெற்றிகரமாக நீக்கப்பட்டது! (Sale entry deleted successfully)")
    } catch (err) {
