...
async function forceSet320Metti() {
  console.log("=== FORCE CLEANING & SETTING EXACT 320 PCS OF METTI IN SUPABASE ===")
  ...
  // Delete ALL existing Metti stock entries
  const { data: delData } = await supabase.from('stock_entries').delete().eq('category_id', cat.id).select()
  console.log(`Deleted ${delData?.length || 0} old Metti entries.`)
  ...
  console.log(`=== SUCCESSFULLY INSERTED EXACT ${totalInsertedPcs} METTI PCS (${totalInsertedWt.toFixed(3)}g) ===`)

  // Check final counts in DB per variant
  ...
  console.log("=== FINAL METTI COUNTS PER VARIANT IN SUPABASE ===")
  console.log(summary)
}
