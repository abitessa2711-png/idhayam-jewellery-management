...
async function test() {
  const { data: cats } = await supabase.from('categories').select('*').eq('name', 'வெள்ளி பொருட்கள்')
  console.log('Category:', cats)
  if (cats && cats.length > 0) {
    const { data: subs } = await supabase.from('subcategories').select('*').eq('category_id', cats[0].id).eq('name', 'டாலர்')
