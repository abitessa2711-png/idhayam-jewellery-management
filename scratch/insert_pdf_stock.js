...
async function insertAllPdfStock() {
  console.log("=== STARTING STOCK INSERTION FOR 13 PDF PAGES ===")
  const { data: dbCategories } = await supabase.from('categories').select('*')
  const { data: dbSubcategories } = await supabase.from('subcategories').select('*')
  const { data: dbVariants } = await supabase.from('variants').select('*')

  let totalInsertedPcs = 0
  let totalInsertedWt = 0

  for (const group of pdfEntries) {
    const category = dbCategories.find(c => c.name === group.cat)
    const subcategory = dbSubcategories.find(s => s.name === group.sub && s.category_id === category?.id)
    const variant = dbVariants.find(v => v.name === group.var && v.category_id === category?.id && v.subcategory_id === (subcategory?.id || null))

    if (!category || !subcategory || !variant) {
      console.error(`MISSING LOOKUP for ${group.cat} -> ${group.sub} -> ${group.var}`)
      continue
    }
  ...
  console.log(`=== SUCCESSFULLY INSERTED ${totalInsertedPcs} Pcs (${totalInsertedWt.toFixed(3)}g) TO LIVE STOCK! ===`)
}

if (process.argv.includes('--execute')) {
  insertAllPdfStock()
} else {
  console.log("Ready to execute when requested.")
}
