...
export async function insertNewItems() {
  ...
  // 2. Valaial (category 4: வளையல், subcategory 8: வகைகள்)
  const { data: catVal } = await supabase.from('categories').select('*').eq('name', 'வளையல்').single()
  const { data: subVal } = await supabase.from('subcategories').select('*').eq('category_id', catVal.id).eq('name', 'வகைகள்').single()
