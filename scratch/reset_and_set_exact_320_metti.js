...
async function resetAndSetCleanMetti() {
  console.log("=== RESETTING METTI STOCK AND INSERTING EXACT 320 PCS ===")
  ...
  // 2. Delete ALL existing Metti entries to clear duplicates
  console.log("Deleting old Metti entries in stock_entries...")
  const { data: delOld } = await supabase.from('stock_entries').delete().eq('category_id', cat.id).select()
  console.log(`Deleted ${delOld?.length || 0} old Metti stock entries.`)

  // 3. Insert exact 320 Metti items
  let totalInsertedCount = 0
  let totalInsertedWeight = 0
  ...
  for (const group of exact320MettiCatalog) {
    ...
    for (const wt of group.weights) {
      ...
      if (error) {
        ...
        totalInsertedCount++
        totalInsertedWeight += wt
      }
    }
  }

  console.log(`=== SUCCESSFULLY INSERTED EXACT ${totalInsertedCount} METTI PCS (${totalInsertedWeight.toFixed(3)}g) ===`)

  // Verify final grand live stock total across all categories
