...
const mettiEntriesToUndo = [
  ...
  { varName: 'சுத்து மெட்டி', weights: [7.880, 5.790, 7.210, 10.590, 7.320, 10.810, 10.230, 5.620, 8.310, 5.970, 8.050, 10.790, 5.640, 7.790, 5.330, 5.300] },
...
async function undoAllMetti() {
  console.log("=== UNDOING / DELETING ALL NEWLY ADDED METTI ITEMS FROM LIVE STOCK ===")
  ...
  for (const group of mettiEntriesToUndo) {
    const { data: variant } = await supabase.from('variants').select('*').eq('category_id', cat.id).eq('subcategory_id', sub.id).eq('name', group.varName).maybeSingle()

    if (!variant) continue

    for (const wt of group.weights) {
      // Find matching entry and delete 1 row matching this weight
      ...
      if (matches && matches.length > 0) {
        ...
        const { error } = await supabase.from('stock_entries').delete().eq('id', rowToDelete.id)
  ...
  console.log(`=== SUCCESSFULLY UNDONE / DELETED ${deletedCount} METTI PCS (${deletedWeight.toFixed(3)}g) ===`)

  // Check updated live stock grand total
  ...
  console.log(`RESTORED LIVE STOCK GRAND TOTAL: ${finalPcs} Pcs | ${finalWt.toFixed(3)}g (~${(finalWt/1000).toFixed(2)} kg)`)
}
